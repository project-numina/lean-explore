...
from lean_explore.mcp.app import mcp_app, AppContext, BackendServiceType
...
async def search(
    ...
    Args:
        ...
        package_filters: An optional list of package names to filter the search
                         results by. For example, `["Mathlib.Analysis", "Mathlib.Order"]`.
                         If None or empty, no package filter is applied.
    ...
    if not api_response_pydantic:
        ...
        empty_response = APISearchResponse(
            query=query, packages_applied=package_filters or [], results=[],
            count=0, total_candidates_considered=0, processing_time_ms=0
        )
