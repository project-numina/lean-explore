...
from lean_explore.models import Declaration, Dependency, StatementGroupDependency
...
def populate_statement_group_dependencies(
    ...
    if not create_tables_flag:
        try:
            with session_factory() as session:
                logger.info("Phase 5: Fetching existing statement_group_dependencies from database...")
            ...
            logger.warning(
                ...
                "Proceeding and relying on DB constraints for duplicates.", e
            )
    ...
    try:
        with session_factory() as session:
            ...
            decl_id_to_sg_id_map = {
                decl_id: sg_id for decl_id, sg_id in results_map if decl_id is not None and sg_id is not None
            }
            logger.info(
            ...
            if not decl_id_to_sg_id_map:
                logger.warning("Phase 5: No declarations found linked to statement groups. Cannot derive group dependencies.")
            ...
            if not declaration_links:
                logger.info("Phase 5: No declaration-level dependencies found. No statement group dependencies to derive.")
                return True

    except SQLAlchemyError as e_fetch:
        logger.error("Phase 5: Database error during initial data fetching: %s", e_fetch, exc_info=True)
        return False
    except Exception as e_unexpected_fetch: # pylint: disable=broad-except
        logger.error("Phase 5: Unexpected error during initial data fetching: %s", e_unexpected_fetch, exc_info=True)
        return False

    new_sg_dependency_payloads: List[Dict[str, Any]] = []
    # This set tracks (source_sg_id, target_sg_id, type) to avoid adding duplicates within the current run,
    # especially if existing_sg_deps_set might not be complete (e.g., if create_tables_flag was true).
    ...
    try:
        with session_factory() as session:
            with tqdm(total=len(declaration_links), desc="Phase 5: Deriving SG Dependencies", unit="decl_link") as pbar:
                for source_decl_id, target_decl_id in declaration_links:
                    ...
                    if source_sg_id == target_sg_id:
                        ...
                        continue # Skip dependencies within the same statement group

                    sg_dep_tuple = (source_sg_id, target_sg_id, DEFAULT_SG_DEPENDENCY_TYPE)

                    if sg_dep_tuple in existing_sg_deps_set or \
                       sg_dep_tuple in processed_in_this_run_sg_deps_set:
                    ...
                    if len(new_sg_dependency_payloads) >= batch_size:
                        try:
                            session.bulk_insert_mappings(StatementGroupDependency, new_sg_dependency_payloads)
                            session.commit()
                            added_count += len(new_sg_dependency_payloads)
                            logger.debug("Phase 5: Committed batch of %d statement group dependencies.", len(new_sg_dependency_payloads))
                            new_sg_dependency_payloads.clear()
                        except IntegrityError:
                            session.rollback()
                            logger.warning(
                                "Phase 5: IntegrityError on batch insert (likely a concurrent duplicate not caught by pre-check). "
                                "Skipping duplicates in this batch and continuing."
                            )
                            # If an integrity error occurs, we might lose the non-duplicates in this specific batch
                            # unless we insert one by one, or re-filter. For now, clearing and continuing.
                            # A more robust approach might filter out the specific duplicates and retry.
                            new_sg_dependency_payloads.clear() # Discard batch on error to be safe
                        except SQLAlchemyError as e_batch:
                            session.rollback()
                            logger.error("Phase 5: Database error during batch insert: %s. Skipping batch.", e_batch)
                ...
                if new_sg_dependency_payloads:
                    try:
                        ...
                        session.commit()
                        added_count += len(new_sg_dependency_payloads)
                        logger.debug("Phase 5: Committed final batch of %d statement group dependencies.", len(new_sg_dependency_payloads))
                        new_sg_dependency_payloads.clear()
                    except IntegrityError:
                        session.rollback()
                        logger.warning("Phase 5: IntegrityError on final batch insert. Some items may not have been added.")
                    except SQLAlchemyError as e_final_batch:
                        session.rollback()
                        logger.error("Phase 5: Database error during final batch insert: %s.", e_final_batch)
            ...
            logger.info(
                ...
                added_count, skipped_due_to_missing_sg_link, skipped_due_to_self_loop,
