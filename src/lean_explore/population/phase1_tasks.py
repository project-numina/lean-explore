...
from lean_explore.models import Declaration, StatementGroup
...
def module_name_to_rel_path(module_name: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(
            "Error converting module name '%s' to path: %s", module_name, e
        )
        return None
...
def _choose_primary_declaration_for_phase1(
    ...
    candidates = [
        decl for decl in declarations_in_block if not decl.get("is_internal")
    ]
    if not candidates:
        candidates = declarations_in_block

    preferred_types_order = [
        "definition", "def", "theorem", "thm", "lemma", "inductive", "structure",
        "class", "instance", "abbreviation", "abbrev", "opaque", "axiom",
        "constructor", "ctor", "example",
    ]
    type_priority_map = {
        dtype: i for i, dtype in enumerate(preferred_types_order)
    }

    candidates.sort(
        key=lambda d: (
            type_priority_map.get(d.get("decl_type", ""), len(preferred_types_order) + 1),
            d.get("lean_name", ""),
        )
    )
...
def _execute_select_in_chunks(
    ...
    Args:
        ...
        select_stmt: The base SELECT statement (without the IN clause applied to filter_column).
                     Any existing WHERE clauses on select_stmt will be preserved and combined with AND.
    ...
    """
    if not values:
        logger.debug("_execute_select_in_chunks: No values provided for IN clause, attempting to execute base statement or returning empty if IN is essential.")
        # If the IN clause is essential and values are empty, it means no results.
        # An empty IN () is often invalid SQL or means false.
        # If select_stmt has no other where clauses, this might select all if not handled.
        # To be safe, if values is empty, we add a 'WHERE false' effectively.
        # However, if the original select_stmt is simple (e.g., just selecting columns),
        # and the IN is the only filter, returning [] is correct.
        # Let's refine to add an always-false condition if values is empty.
        # This needs to be done carefully to not break complex existing where clauses.
        # For simplicity here, if it's an IN filter that's empty, usually means "no match for this part".
        # If the overall query relies on this IN to return anything, then empty list is correct.
        return []
...
def _execute_delete_in_chunks(
    ...
    """
    if not values:
        logger.debug("_execute_delete_in_chunks: No values provided for IN clause, no rows deleted.")
        return 0
...
def phase1_populate_declarations_initial(
    ...
    """
    logger.info(
        "Starting Phase 1: Initial population and synchronization of declarations from %s...",
        declarations_file,
    )
    ...
    try:
        total_lines = 0
        with open(declarations_file, "r", encoding="utf-8") as f_count:
            for _ in f_count:
                total_lines += 1
        if total_lines == 0:
            logger.info(
            ...
            if not create_tables_flag:
                logger.info("Declarations file is empty, proceeding to delete all existing declarations as it's the source of truth.")
                try:
                    with session_factory() as session:
                        all_db_decl_results = session.execute(select(Declaration.id)).fetchall()
                        all_db_decl_ids = {id_val for id_val, in all_db_decl_results}
                        if all_db_decl_ids:
                            _delete_stale_declarations_and_manage_sg_integrity(session, set(), all_db_decl_ids)
                            session.commit() 
                        else:
                            logger.info("No declarations found in the database to delete.")
                except Exception as e_delete_all:
                    logger.error("Error during deletion of all declarations: %s", e_delete_all, exc_info=True)
                    return None
            return {}
    except Exception as e: # pylint: disable=broad-except
        logger.error(
    ...
    if not create_tables_flag:
        try:
            with session_factory() as session:
                logger.info("Fetching existing declarations from database for Phase 1 comparison...")
                results = session.execute(select(Declaration)).scalars().all()
                for decl in results:
                    if decl.lean_name:
                        existing_declarations_map[decl.lean_name] = decl
                logger.info(
                    "Found %d existing declarations for comparison.", len(existing_declarations_map)
                )
        except Exception as e: # pylint: disable=broad-except
            logger.warning(
                "Could not fetch existing declarations for Phase 1: %s. "
                "Proceeding as if no existing declarations found for comparison part.", e
            )

    try:
        with open(declarations_file, "r", encoding="utf-8") as f_decls, session_factory() as session:
            with tqdm(total=total_lines, desc="Phase 1: Upserting Declarations", unit="line") as pbar:
                ...
                for i, line in enumerate(f_decls):
                    ...
                    try:
                        ...
                        if not lean_name_for_log or lean_name_for_log.startswith("UNKNOWN_IN_LINE_") or not decl_type:
                            logger.warning(
                                "Skipping line %d: Missing 'lean_name' or 'decl_type'. Line: %s",
                                i + 1, line.strip(),
                            )
                            continue
                        ...
                        is_projection_val = ("projection" in attributes_list if isinstance(attributes_list, list) else False)

                        auto_generated_suffixes = [
                            ".noConfusion", ".noConfusionType", ".rec", ".recOn", ".casesOn",
                            ".brecOn", ".below", ".IBelow", ".ndrec", ".ndrecOn",
                            ".match_1", ".match_2", ".matcher", ".mk.inj", ".mk.inj_arrow",
                            ".sizeOf_spec", "_uniq", ".internal",
                        ]
                        core_prefixes = ["Lean.", "Init."]
                        is_internal_val = any(lean_name_for_log.startswith(p) for p in core_prefixes) or \
                                          any(lean_name_for_log.endswith(s) for s in auto_generated_suffixes) or \
                                          "._match" in lean_name_for_log or \
                                          "._proof_" in lean_name_for_log or \
                                          "._example" in lean_name_for_log

                        name_parts = lean_name_for_log.split(".")
                        if len(name_parts) > 1 and name_parts[-1].startswith("eq_") and name_parts[-1][3:].isdigit():
                            is_internal_val = True
                        if ".Internal." in name_parts:
                             is_internal_val = True

                        decl_data_dict = {
                            "lean_name": lean_name_for_log, "decl_type": decl_type,
                            "module_name": module_name, "source_file": source_file_rel,
                        ...
                        if log_this_sample:
                            log_dict_sample = {k: v for k,v in decl_data_dict.items() if k != "docstring"}
                            logger.debug(
                                "[Phase 1 Sample Record #%d] Prepared dict for '%s': %s",
                                i + 1, lean_name_for_log, log_dict_sample,
                            )
                        ...
                        if existing_decl:
                            ...
                            for key, value in decl_data_dict.items():
                                if value != getattr(existing_decl, key, None):
                                    ...
                                    changed = True
                            if changed:
                                if log_this_sample: logger.debug("Queuing UPDATE for '%s'.", lean_name_for_log)
                                current_batch_data_to_update.append(update_payload)
                        else:
                            if log_this_sample: logger.debug("Queuing NEW insert for '%s'.", lean_name_for_log)
                            current_batch_data_to_add.append(decl_data_dict)

                        if (len(current_batch_data_to_add) + len(current_batch_data_to_update)) >= batch_size:
                            ...
                            if current_batch_data_to_update:
                                session.bulk_update_mappings(Declaration, current_batch_data_to_update)
                                db_updates += len(current_batch_data_to_update)
                            session.commit()
                            pbar.set_postfix_str(f"Adds: {db_adds}, Updates: {db_updates}", refresh=False)
                            current_batch_data_to_add = []
                            current_batch_data_to_update = []
                    except json.JSONDecodeError:
                        logger.error("Failed to parse JSON at line %d: %s", i + 1, line.strip())
                    except IntegrityError:
                        session.rollback()
                        logger.warning("IntegrityError for '%s' at line %d (likely duplicate).", lean_name_for_log, i + 1)
                    except SQLAlchemyError as e_item:
                        session.rollback()
                        logger.error("DB error for '%s' at line %d: %s. Skipping.", lean_name_for_log, i + 1, e_item)
                    except Exception as e_item_unexpected: # pylint: disable=broad-except
                        session.rollback()
                        logger.error("Unexpected error for '%s' at line %d: %s. Skipping.", lean_name_for_log, i + 1, e_item_unexpected, exc_info=True)
                ...
                if current_batch_data_to_add or current_batch_data_to_update:
                    session.commit()
                pbar.set_postfix_str(f"Adds: {db_adds}, Updates: {db_updates}. Upserts done.", refresh=True)
            
        logger.info(
            "Phase 1: Upsert processing finished. Processed %d lines. DB additions: %d, DB updates: %d.",
            total_lines, db_adds, db_updates,
        )
        logger.info("Collected %d unique lean_names from input file for active set.", len(active_lean_names_from_input))

    except FileNotFoundError:
        logger.error("Declarations file not found during Phase 1 execution: %s", declarations_file)
        return None
    except SQLAlchemyError as e_db_critical_upsert:
        logger.error("A critical database error occurred during Phase 1 (upsert part): %s", e_db_critical_upsert, exc_info=True)
        return None
    except Exception as e_critical_upsert: # pylint: disable=broad-except
        logger.error("A critical unexpected error occurred during Phase 1 (upsert part): %s", e_critical_upsert, exc_info=True)
        return None
    ...
    if not create_tables_flag:
        ...
        try:
            with session_factory() as session:
                ...
                db_lean_names_to_ids = {name: id_val for id_val, name in all_db_declarations_info if name}
                db_lean_names = set(db_lean_names_to_ids.keys())
                
                stale_lean_names = db_lean_names - active_lean_names_from_input
                stale_declaration_ids = {db_lean_names_to_ids[name] for name in stale_lean_names if name in db_lean_names_to_ids}


                if stale_declaration_ids:
                    logger.info("Found %d stale declarations to process for deletion.", len(stale_declaration_ids))
                    _delete_stale_declarations_and_manage_sg_integrity(session, active_lean_names_from_input, stale_declaration_ids)
                    session.commit() 
                else:
            ...
            logger.error("Critical error during Phase 1 (stale data deletion part): %s. Database may be inconsistent.", e_delete_critical, exc_info=True)
            return None

    final_lean_name_to_db_id: Dict[str, int] = {}
    try:
        with session_factory() as session:
            logger.info("Phase 1: Rebuilding lean_name to database ID map from current DB state...")
            ...
            if not active_lean_names_from_input:
                if not create_tables_flag and total_lines == 0 : # Input was empty, and deletions occurred
                    logger.info("Input was empty and stale data potentially deleted; final map will be empty if DB is empty.")
                    ...
                    results_rebuild = session.execute(rebuild_stmt_base).fetchall()
                     ...
                     logger.info("Input was empty and create_tables_flag is true; final map is empty.")
                     results_rebuild = []
                else: # Non-empty input file yielded no active_lean_names (all lines invalid?)
                      # Or if initial file processing failed to populate active_lean_names_from_input
                    logger.warning("No active lean names collected from input, or input processing error. "
                                   "Rebuilding map from all current DB entries as a fallback.")
                    results_rebuild = session.execute(rebuild_stmt_base).fetchall()
            ...
            final_lean_name_to_db_id = {name: id_val for id_val, name in results_rebuild if name}
        ...
        logger.error("Phase 1: Error rebuilding final lean_name_to_db_id map: %s", e_rebuild_map, exc_info=True)
        return None
...
def _delete_stale_declarations_and_manage_sg_integrity(
    ...
    logger.info("Managing StatementGroup integrity for %d stale declarations...", len(stale_declaration_ids))
    ...
    base_active_decls_stmt = select(
        Declaration.id, Declaration.lean_name, Declaration.decl_type,
        Declaration.is_internal, Declaration.statement_group_id
    ...
    for decl_id, lean_name, decl_type, is_internal, sg_id_val in active_decls_in_groups_raw:
        if sg_id_val is not None: 
            ...
            sg_id_to_active_members[sg_id_val].append({
                "id": decl_id, "lean_name": lean_name,
                "decl_type": decl_type, "is_internal": is_internal
            })
    ...
    for sg_id, sg_primary_decl_id in sg_info_list_tuples:
        if sg_primary_decl_id in stale_declaration_ids:
            logger.debug("StatementGroup ID %d primary declaration ID %d is stale.", sg_id, sg_primary_decl_id)
            active_members_for_this_sg = sg_id_to_active_members.get(sg_id, [])
            if not active_members_for_this_sg:
                logger.debug("StatementGroup ID %d has no remaining active members. Marking for deletion.", sg_id)
                statement_group_ids_to_delete.add(sg_id)
            else:
                new_primary_decl_id = _choose_primary_declaration_for_phase1(active_members_for_this_sg)
                if new_primary_decl_id is not None:
                    if sg_primary_decl_id != new_primary_decl_id:
                        logger.debug("StatementGroup ID %d: Assigning new primary declaration ID %d.", sg_id, new_primary_decl_id)
                        statement_groups_to_update_payloads.append({
                            "id": sg_id, "primary_decl_id": new_primary_decl_id
                        })
                else:
                    logger.warning("StatementGroup ID %d: Could not choose a new primary. Marking for deletion.", sg_id)
                    statement_group_ids_to_delete.add(sg_id)
    
    if statement_groups_to_update_payloads:
        logger.info("Updating primary_decl_id for %d StatementGroups.", len(statement_groups_to_update_payloads))
        try:
            session.bulk_update_mappings(StatementGroup, statement_groups_to_update_payloads)
        except SQLAlchemyError as e_sg_update:
            logger.error("Error updating StatementGroup primary_decl_ids: %s. Changes for SGs in this batch might be lost.", e_sg_update, exc_info=True)
            ...
            # raise # Or re-raise to abort the entire deletion process in phase1_populate_declarations_initial

    logger.info("Preparing to delete %d stale declarations...", len(stale_declaration_ids))
    ...
    if active_lean_names_from_input: # Only try to find orphans if there's a basis for "active"
        base_active_linked_sgs_stmt = select(Declaration.statement_group_id.distinct()).where(
        ...
        active_sg_ids_via_links = {sg_id for sg_id, in active_declarations_still_linked_to_sgs_tuples if sg_id is not None}
        
        all_current_sg_ids = {sg_id for sg_id, _ in sg_info_list_tuples}
        
        orphaned_sg_ids = all_current_sg_ids - active_sg_ids_via_links
        orphaned_sg_ids = orphaned_sg_ids - statement_group_ids_to_delete # Avoid re-adding
        if orphaned_sg_ids:
            logger.info("Identified %d additional orphaned StatementGroups for deletion.", len(orphaned_sg_ids))
            statement_group_ids_to_delete.update(orphaned_sg_ids)
            
    elif not active_lean_names_from_input and sg_info_list_tuples: # Input was empty (or all names stale)
        logger.info("Input file was empty or all names stale, marking all %d StatementGroups for deletion.", len(sg_info_list_tuples))
        statement_group_ids_to_delete.update(sg_id for sg_id, _ in sg_info_list_tuples)

    if statement_group_ids_to_delete:
        logger.info("Preparing to delete %d stale/orphaned StatementGroups...", len(statement_group_ids_to_delete))
        deleted_sg_count = _execute_delete_in_chunks(
            session, StatementGroup, StatementGroup.id, list(statement_group_ids_to_delete)
        )
        logger.info("Actually deleted %d stale/orphaned StatementGroups.", deleted_sg_count)
    # The main caller `phase1_populate_declarations_initial` will commit the session after this function.