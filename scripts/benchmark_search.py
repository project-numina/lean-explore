...
try:
    ...
    from lean_explore.search import (
    ...
    if 'tqdm' in str(e).lower():
        print("Warning: tqdm library not found. Progress bars will not be shown. "
              "Please install it: pip install tqdm", file=sys.stderr)
        ...
        print(
            ...
            "Ensure 'lean_explore' is structured correctly, all dependencies are installed "
            "(e.g., via 'pip install -r requirements.txt' or 'pip install .'), "
            "and that this script is run from a context where 'src' is discoverable.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def load_test_queries(file_path: pathlib.Path) -> List[str]:
    ...
    try:
        ...
        logger.error("Failed to load test queries from %s: %s", file_path, e, exc_info=True)
        return []
...
def parse_benchmark_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Filename of test queries (expected in scripts/ directory or provide full path).",
    )
    parser.add_argument(
        "--benchmark_runs",
        type=int,
...
def main():
    ...
    try:
        ...
        if not all([raw_db_url_from_config, embedding_model_name, faiss_idx_path_str, faiss_map_path_str]):
            missing_configs = [
                k for k, v in {
                    "Database URL": raw_db_url_from_config, "Embedding Model Name": embedding_model_name,
                    "FAISS Index Path": faiss_idx_path_str, "FAISS Map Path": faiss_map_path_str
                }.items() if not v
            ]
            raise ValueError(f"Missing critical configurations: {', '.join(missing_configs)}")
        ...
        logger.error("Failed to load or validate application configuration: %s", e, exc_info=True)
        sys.exit(1)
    ...
    try:
        ...
        if s_transformer_model is None: sys.exit(1)
        faiss_idx, id_map = load_faiss_assets(str(resolved_idx_path), str(resolved_map_path))
        if faiss_idx is None or id_map is None: sys.exit(1)
    ...
    queries_file_abs_path = (pathlib.Path(__file__).resolve().parent / queries_file_path_str
    ...
    if not test_queries:
        logger.error("No test queries loaded. Ensure '%s' exists and is readable.", queries_file_abs_path)
    ...
    try:
        search_module_logger.setLevel(logging.WARNING) # Suppress INFO from lean_explore.search

        # Warm-up phase has been removed.

        # Execution Phase
        if args.benchmark_runs <= 0 or num_unique_queries == 0:
            logger.info("No benchmark runs specified or no queries loaded. Exiting benchmark execution.")
            if engine: engine.dispose()
            sys.exit(0)
            
        total_benchmark_queries_to_process = args.benchmark_runs * num_unique_queries
        logger.info(
            "Starting benchmark phase: %d passes over %d unique queries (%d total benchmark queries to process)...",
            args.benchmark_runs, num_unique_queries, total_benchmark_queries_to_process
        )
        ...
        with tqdm(total=total_benchmark_queries_to_process, desc="Benchmark Queries", unit="query", ncols=100) as pbar:
                ...
                try:
                    with SessionLocal() as session:
                        _results = perform_search(
                            session=session, query_string=query, model=s_transformer_model,
                            faiss_index=faiss_idx, text_chunk_id_map=id_map, faiss_k=faiss_k,
                            pagerank_weight=pagerank_weight, text_relevance_weight=text_relevance_weight,
                    ...
                    logger.error("[Benchmark] Error during query '%s': %s. Skipping.", query, e) # No exc_info to keep tqdm clean
                pbar.update(1)
        
        benchmark_overall_end_time = time.perf_counter()
        total_elapsed_time_seconds = benchmark_overall_end_time - benchmark_overall_start_time

        logger.info("Benchmark phase completed.") # This log will appear after tqdm finishes
        print("\n--- Benchmark Results ---")
        if benchmark_total_queries_processed_successfully == 0:
            print("No queries were successfully processed in the benchmark phase.")
        else:
            avg_latency_seconds = sum(all_query_latencies) / benchmark_total_queries_processed_successfully
            qps = benchmark_total_queries_processed_successfully / total_elapsed_time_seconds if total_elapsed_time_seconds > 0 else float('inf')
            qpm = qps * 60
            print(f"Total Queries Attempted (Benchmark Phase):  {total_benchmark_queries_to_process}")
            print(f"Total Queries Processed Successfully:       {benchmark_total_queries_processed_successfully}")
            print(f"Total Time Taken (Benchmark Phase):         {total_elapsed_time_seconds:.3f} seconds")
            print(f"Queries Per Second (QPS, successful):       {qps:.2f}")
            print(f"Queries Per Minute (QPM, successful):       {qpm:.2f}")
            print(f"Average Latency per Query (successful):     {avg_latency_seconds:.4f} seconds")
            if all_query_latencies:
                ...
                if len(all_query_latencies) > 0:
                    ...
                    p90_lat = all_query_latencies[min(int(len(all_query_latencies) * 0.90), len(all_query_latencies)-1)]
                    p99_lat = all_query_latencies[min(int(len(all_query_latencies) * 0.99), len(all_query_latencies)-1)]
        ...
        logger.debug("Restored 'lean_explore.search' logger level to original: %s", original_search_logger_level)
        if engine:
            engine.dispose()
