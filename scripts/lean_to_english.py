...
from dataclasses import dataclass, field # Removed unused 'field'
...
def load_group_graph_data(
    ...
    Dict[int, GroupData], Set[int], Dict[int, int], Dict[int, List[int]], Dict[int, List[int]]
]:
    ...
    for group in db_groups:
        ...
        group_data_item = GroupData(
            ...
            informal_description=(
                None if startover else group.informal_description
            ),
        ...
        if startover or not group.informal_description:
            groups_to_process.add(group.id)
    ...
    group_in_degree: Dict[int, int] = {gp_id: 0 for gp_id in groups_to_process}
    group_successors: Dict[int, List[int]] = {
        gp_id: [] for gp_id in groups_to_process
    }
    ...
    stmt_group_deps_query = select(
        ...
        StatementGroupDependency.source_statement_group_id.in_(groups_to_process),
        StatementGroupDependency.target_statement_group_id.in_(groups_to_process)
    )
    direct_group_dependencies = session.execute(stmt_group_deps_query).all()
    logger.info("Loaded %d group dependency links for processing candidates.", len(direct_group_dependencies))
    ...
    for dependent_sg_id, prerequisite_sg_id in direct_group_dependencies:
        if dependent_sg_id == prerequisite_sg_id:
            logger.warning("Ignoring self-dependency for group ID %d from statement_group_dependencies.", dependent_sg_id)
            continue
        ...
        if not dependent_group_data or not prerequisite_group_data:
            logger.warning(
                "Missing GroupData for group IDs %d or %d from statement_group_dependencies. Skipping edge.",
                dependent_sg_id, prerequisite_sg_id
            )
            continue

        dependent_category = get_library_category(dependent_group_data.source_file)
        prerequisite_category = get_library_category(prerequisite_group_data.source_file)

        if dependent_category == "THEORY" and prerequisite_category == "FOUNDATIONAL":
            logger.debug(
                "Ignoring dependency edge from FOUNDATIONAL group %d (%s) "
                "to THEORY group %d (%s) for graph building (from statement_group_dependencies).",
                prerequisite_sg_id,
        ...
        if edge not in processed_edges:
            group_in_degree[dependent_sg_id] = group_in_degree.get(dependent_sg_id, 0) + 1
            group_successors.setdefault(prerequisite_sg_id, []).append(dependent_sg_id)
            group_predecessors.setdefault(dependent_sg_id, []).append(prerequisite_sg_id)
            processed_edges.add(edge)
            valid_group_dependency_count += 1

    logger.info(
        "Built group graph with %d unique inter-group dependencies among processing candidates.",
...
def format_prerequisites_for_prompt(
    ...
    if current_group_data:
        current_category = get_library_category(current_group_data.source_file) or "OTHER"

    if not prerequisite_group_ids:
        return "This statement block has no direct prerequisite blocks that required processing.\n"

    header = "This statement block depends on the following concepts (from prerequisite blocks):\n"
    prompt_lines = [header]
    ...
    for prereq_id in sorted(prerequisite_group_ids):
        ...
        if not prereq_data:
            logger.warning(
                "Could not find data for prerequisite group ID %d during prompt formatting.",
                prereq_id,
            )
            continue
        ...
        if current_category == "THEORY" and prereq_category == "FOUNDATIONAL":
            logger.debug(
                "Skipping FOUNDATIONAL prerequisite group %d (%s) for THEORY group %d (%s) in prompt.",
                prereq_id,
    ...
    for prereq_id_to_show in filtered_prereq_ids:
        ...
        desc = generated_group_translations.get(prereq_id_to_show, prereq_data.informal_description if prereq_data else None)
        if not desc or not desc.strip():
            desc = "[Description not available]"
        elif len(desc) > max_chars_per_desc:
            desc = desc[:max_chars_per_desc].rsplit(" ", 1)[0] + "..."

        line = f"- **{prereq_data.primary_decl_name}** ({prereq_data.primary_decl_type} block): {desc}\n"
    ...
    if items_added < num_shown_or_elligible and limit_reason:
        ...
        ellipsis_line = f"- ... (and {remaining} more eligible prerequisite block(s) not shown due to {limit_reason})\n"
        ...
        if len(prompt_lines) == 1 and prompt_lines[0] == header:
            prompt_lines.append("- (All direct prerequisites were filtered based on library category rules.)\n")
    elif num_shown_or_elligible == 0 and num_total_prereqs_considered == 0 :
        if len(prompt_lines) == 1 and prompt_lines[0] == header :
            return "This statement block has no direct prerequisite blocks that required processing or were eligible for display.\n"
...
async def _process_single_statement_group_with_llm(
    ...
    if not current_group:
        return group_id, None, ValueError(f"Data for StatementGroup ID {group_id} not found.")
    if not current_group.statement_text:
        logger.warning(
            "StatementGroup %d (%s) has no statement_text, cannot generate description.",
            group_id,
    ...
    try:
        ...
        if is_test_mode:
            print(
                f"\n--- LLM PROMPT for Group: {current_group.primary_decl_name} (ID: {group_id}) ---\n"
        ...
        if generated_text and generated_text.strip():
            ...
            if is_test_mode:
                print(
                    f"\n--- Test Output for Group: {current_group.primary_decl_name} (ID: {group_id}) ---\n"
...
async def process_statement_groups_with_llm(
    ...
    mode_desc_str = f"Test Mode (limit {test_limit}, NO DB writes)" if is_test_mode else "Live Mode"
    if startover:
    ...
    prompt_template_path_str = lean_to_english_cfg.get("prompt_template_path", DEFAULT_PROMPT_TEMPLATE_PATH)
    try:
        ...
        logger.error(
            ...
            prompt_template_path_str, template_path
        )
        return False
    except Exception as e:
        logger.error("Error loading prompt template %s: %s", template_path if 'template_path' in locals() else prompt_template_path_str, e, exc_info=True)
        return False

    max_concurrent_calls = lean_to_english_cfg.get("max_concurrent_llm_calls", DEFAULT_MAX_CONCURRENT_LLM_CALLS)
    if not (isinstance(max_concurrent_calls, int) and max_concurrent_calls > 0):
        logger.warning("Invalid max_concurrent_llm_calls '%s', using default %d.", max_concurrent_calls, DEFAULT_MAX_CONCURRENT_LLM_CALLS)
    ...
    if max_gather_batch is not None and not (isinstance(max_gather_batch, int) and max_gather_batch > 0) :
        logger.warning("Invalid max_gather_batch_size '%s', applying no limit.", max_gather_batch)
    ...
    try:
        ...
        with engine.connect() as connection: # Verify connection early
            ...
            required_tables = [Declaration.__tablename__, StatementGroup.__tablename__, StatementGroupDependency.__tablename__]
            if not all(inspector.has_table(t) for t in required_tables):
                missing = [t for t in required_tables if not inspector.has_table(t)]
                logger.error("Database missing one or more required tables: %s", missing)
                return False
            sg_columns = [col["name"] for col in inspector.get_columns(StatementGroup.__tablename__)]
            if "informal_description" not in sg_columns:
                logger.error("DB table '%s' missing 'informal_description' column.", StatementGroup.__tablename__)
                return False
    ...
    try:
        with SessionLocal() as session_obj_for_processing:
            ...
            pbar_total = min(test_limit, total_to_process_initially) if is_test_mode else total_to_process_initially
            pbar = tqdm(total=pbar_total, desc="Processing Groups", unit="group")
            initial_cost_str = f"${client.cost_tracker.get_total_cost():.4f}"
            pbar.set_postfix_str(f"Cost: {initial_cost_str} | OK: 0 | Fail: 0 | {mode_desc_str.split(' [')[0]}")

            # Phase 1: Topological Sort
            logger.info("Starting topological sort phase...")
            current_processing_queue = deque(sorted([
                gid for gid in groups_to_process_ids if group_in_degree.get(gid, 0) == 0
            ]))
            ...
            while current_processing_queue:
                if is_test_mode and processed_count >= test_limit:
                    logger.info("Reached test limit of %d items during %s phase.", test_limit, phase_name)
                    break
                ...
                if is_test_mode:
                    ...
                    current_gather_size = min(current_gather_size, remaining_in_test_limit)

                if current_gather_size <= 0: break 

                current_batch_ids = [current_processing_queue.popleft() for _ in range(current_gather_size)]
                launched_task_ids.update(current_batch_ids)
                
                tasks = [
                    _process_single_statement_group_with_llm(
                        gid, all_group_data, group_predecessors,
                        generated_group_translations, client, llm_semaphore,
                        prompt_template, is_test_mode,
                ...
                for res_item in results_from_gather:
                    if isinstance(res_item, Exception):
                        ...
                        logger.error("Asyncio.gather task raised an exception: %s", res_item)
                        continue
                ...
                if pbar: pbar.update(len(current_batch_ids)) # Update based on tasks launched
                cost_str = f"${client.cost_tracker.get_total_cost():.4f}"
                pbar.set_postfix_str(
                    f"Cost: {cost_str} | OK: {translation_success_count} | "
                    f"Fail: {translation_failure_count} | {mode_desc_str.split(' [')[0]}"
                )

                if not is_test_mode and len(pending_db_updates) >= commit_batch_size:
                    logger.info("Committing batch of %d group updates...", len(pending_db_updates))
                    update_stmt_mappings = [{"id": gid, "informal_description": desc} for gid, desc in pending_db_updates]
                    session_obj_for_processing.execute(update(StatementGroup), update_stmt_mappings)
                        ...
                        for successor_gid in group_successors.get(proc_gid, []):
                            if successor_gid in group_in_degree:
                                ...
                                if group_in_degree[successor_gid] == 0:
                                    if successor_gid not in launched_task_ids and successor_gid not in current_processing_queue:
                                        current_processing_queue.append(successor_gid)
            
            if phase_name == "Topological": # End of topological while loop
                 logger.info("Topological sort phase finished. Items attempted in this phase: %d.", len(launched_task_ids))
            ...
            if unlaunched_ids:
                if is_test_mode and processed_count >= test_limit:
                    logger.info("Test limit reached, skipping fallback phase for %d items.", len(unlaunched_ids))
                else:
                    logger.warning(
                        "%d group(s) remain unprocessed after topological sort, likely due to cycles. "
                        "Attempting to process these now by breaking cycles heuristically (processing in ID order).",
                        len(unlaunched_ids)
                    )
                    phase_name = "Fallback" # For logging or conditional behavior if any
                    current_processing_queue = deque(sorted(list(unlaunched_ids))) # New queue for fallback
                    ...
                    while current_processing_queue: # Loop for fallback items
                        ...
                        current_batch_ids = [current_processing_queue.popleft() for _ in range(current_gather_size)]
                        # These IDs are from unlaunched_ids, so they add to launched_task_ids now
                        launched_task_ids.update(current_batch_ids) 
                        ...
                        # Re-use result processing logic (identical to topological phase)
                        for res_item in results_from_gather:
                            if isinstance(res_item, Exception):
                                translation_failure_count += 1
                                logger.error("Asyncio.gather task raised an exception during fallback: %s", res_item)
                                continue
                            res_group_id, translation, error_obj = res_item
                            if error_obj:
                                translation_failure_count += 1
                        ...
                        if pbar: pbar.update(len(current_batch_ids))
                        cost_str = f"${client.cost_tracker.get_total_cost():.4f}"
                        pbar.set_postfix_str(
                            f"Cost: {cost_str} | OK: {translation_success_count} | "
                            f"Fail: {translation_failure_count} | {mode_desc_str.split(' [')[0]}"
                        ...
                        if not is_test_mode and len(pending_db_updates) >= commit_batch_size:
                            logger.info("Committing batch of %d group updates (fallback)...", len(pending_db_updates))
                            update_stmt_mappings = [{"id": gid, "informal_description": desc} for gid, desc in pending_db_updates]
                            session_obj_for_processing.execute(update(StatementGroup), update_stmt_mappings)
                            session_obj_for_processing.commit()
                            pending_db_updates.clear()
            ...
            if not is_test_mode and pending_db_updates:
                logger.info("Committing final batch of %d group updates...", len(pending_db_updates))
                final_update_mappings = [
                    {"id": gid, "informal_description": desc} for gid, desc in pending_db_updates
                ]
                session_obj_for_processing.execute(update(StatementGroup), final_update_mappings)
                session_obj_for_processing.commit()
                pending_db_updates.clear()

    except SQLAlchemyError as db_err:
        logger.error("Database error during processing: %s", db_err, exc_info=True)
        if session_obj_for_processing and session_obj_for_processing.is_active : session_obj_for_processing.rollback()
        return False
    except Exception as e:
        ...
        if client and client.cost_tracker:
            ...
            logger.info("Total Estimated Cost: $%.4f", summary.get("total_estimated_cost", 0.0))
            logger.info("LLM Calls Attempted (Processed Count): %d", processed_count) # Total tasks created
            logger.info("LLM Translations Succeeded: %d", translation_success_count)
            logger.info("LLM Calls Failed or Yielded No Translation: %d", translation_failure_count)
    ...
    if is_test_mode:
        logger.info(
            "Test mode finished. Attempted %d items (limit was %d). Successes: %d, Failures: %d.",
            processed_count, test_limit or 0, translation_success_count, translation_failure_count
        )
        return True 

    # In live mode, success means all initially targeted groups were ATTEMPTED.
    if processed_count >= total_to_process_initially:
        if translation_failure_count == 0 and translation_success_count == total_to_process_initially:
            logger.info(
                "Live mode: Successfully processed and translated all %d identified groups.",
                total_to_process_initially
            )
        elif translation_success_count + translation_failure_count == total_to_process_initially : # All attempted had some outcome
             logger.warning(
                "Live mode: Attempted all %d identified groups. %d translations succeeded, %d LLM calls failed/yielded no translation.",
                total_to_process_initially, translation_success_count, translation_failure_count
            )
        else: # Should not happen if processed_count >= total_to_process_initially
            logger.info(
                "Live mode: Processed %d groups. Successes: %d, Failures: %d. Initial total: %d",
                processed_count, translation_success_count, translation_failure_count, total_to_process_initially
            )
    ...
    logger.error(
        ...
        processed_count, total_to_process_initially, translation_success_count, translation_failure_count
    )
    return False
...
async def main_async():
    ...
    if args.db_url.startswith("sqlite:///"):
        ...
        if db_path_str != ":memory:":
            ...
            if not db_file.exists() and not args.test : # Only warn if not test mode, as test might not need existing DB
    ...
    if success:
        logger.info("--- Processing Completed (%s%s) ---", current_mode_desc, startover_msg_part)
        sys.exit(0)
    else:
        logger.error("--- Processing FAILED or Incomplete (%s%s) ---", current_mode_desc, startover_msg_part)
        sys.exit(1)
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(
            "An unexpected critical error occurred in main execution: %s", e, exc_info=True
        )
        sys.exit(1)