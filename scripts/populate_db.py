...
# A module-level logger can be obtained using logging.getLogger(__name__) if needed elsewhere.
        ...
        try:
            ...
            if raw_path_str:
                ...
                if resolved_toolchain_base.is_dir():
                    config_logger.info(
                        "Successfully read and derived toolchain source base from %s: %s",
                        config_file,
                    ...
                    config_logger.warning(
                        "Path derived from 'toolchainCoreSrcDir' in %s ('%s') is not a valid directory.",
                        config_file,
                        resolved_toolchain_base,
                    )
            else:
            ...
            config_logger.warning(
                "Error decoding JSON from %s: %s", config_file, jde
            )
        except Exception as e:  # pylint: disable=broad-except
            config_logger.warning(
                "Error reading or processing %s: %s", config_file, e
            )
    else:
        config_logger.info(
            "Extractor config file not found at %s. "
            "Toolchain source path must be provided via --lean-toolchain-src if needed.",
            config_file
        )
    return None
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="SQLAlchemy database URL. Defaults to an SQLite DB at 'data/lean_explore_data.db'.",
    )
    parser.add_argument(
        "--create-tables",
        action="store_true",
        help="Drop existing tables (if any) and create new ones. "
             "CAUTION: DELETES ALL EXISTING DATA IN THE TARGET DATABASE.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Absolute base path to Lean toolchain's .lean source files (e.g., for Init, Lean, Std). "
             "Overrides auto-detection from 'extractor/extractor_config.json'. "
             "Path should point to the directory containing 'Init', 'Lean', 'Std' subdirectories (e.g., .../src/lean/).",
    )
    parser.add_argument(
        "--lean-lake-src",
        type=Path,
        default=DEFAULT_LAKE_PACKAGES_SRC_BASE,
        help="Absolute base path to the directory containing Lake package sources (e.g., 'mathlib', 'batteries'). "
             "Typically '.lake/packages/' within a Lean project. Used in Phase 2.",
    )
    parser.add_argument(
        "--ast-json-base",
        type=Path,
        default=DEFAULT_AST_JSON_BASE_ABS,
        help="Absolute base path to .ast.json files (e.g., 'data/AST/'). Used in Phase 2.",
    )
...
def main():
    ...
    script_logger = logging.getLogger(__name__) # Logger specific to this script's messages
    ...
    if args.lean_toolchain_src:
        ...
        script_logger.info(
            "Attempting to determine Lean toolchain source base from 'extractor/extractor_config.json'..."
        )
        lean_toolchain_src_abs = get_toolchain_src_from_config(PROJECT_ROOT, script_logger)
        if not lean_toolchain_src_abs:
            script_logger.warning(
                "Lean toolchain source base not provided via --lean-toolchain-src and "
                "could not be determined from config. Phase 2 may not find toolchain .lean files."
            )
    ...
    if not dependencies_file_abs.is_file():
        # This is not immediately critical if only running early phases, but log prominently.
        script_logger.warning(
            "Dependencies file not found: %s. Phase 4 (dependency population) will be impacted.",
    ...
    if lean_lake_packages_root_abs and not lean_lake_packages_root_abs.is_dir():
        script_logger.warning(
            "Specified Lake packages source base is not a directory: %s. "
            "Phase 2 impact on package files.",
    ...
    try:
        ...
        if args.create_tables:
            script_logger.warning(
                "Option --create-tables: DROPPING existing tables and CREATING new ones."
            )
            ...
            with engine.connect() as connection:
                script_logger.info(
                    "Database connection verified (existing tables will be used/updated)."
                )
        ...
        if not lean_name_to_id_map:
            script_logger.warning(
                "Phase 1: Declaration map is empty (no declarations processed or found). "
                "Subsequent phases might have limited or no data to process."
            )
        else:
            script_logger.info(
        ...
        if lean_lake_packages_root_abs and lean_lake_packages_root_abs.is_dir():
            ...
            for package_dir in lean_lake_packages_root_abs.iterdir():
                if package_dir.is_dir():
                    ...
                    script_logger.debug("Added Lake package source base for Phase 2: %s", package_dir)
        
        if not lean_source_bases_for_phase2:
             script_logger.warning(
                "Phase 2: No valid .lean source base paths configured or found. "
                "Source text refinement will likely be skipped for most declarations."
            )
        ...
        if not success_phase2:
            script_logger.warning(
                "Phase 2: Refinement may have encountered errors. Review logs. Subsequent data may be impacted."
            )
        else:
            script_logger.info("Phase 2: Declaration Source Info Refinement processing completed.")
        ...
        if not success_phase3:
            script_logger.warning(
                "Phase 3: Grouping may have encountered errors. Review logs. Declarations may not be optimally grouped."
            )
        else:
        ...
        if not lean_name_to_id_map: # Check if Phase 1 yielded any declarations
            script_logger.warning(
                "Skipping Phase 4: Dependency Population (declaration map from Phase 1 is empty)."
            )
        elif not dependencies_file_abs.is_file():
            script_logger.warning(
                "Skipping Phase 4: Dependency Population (dependencies file not found at %s).",
                dependencies_file_abs
            )
        else:
            ...
            if not success_phase4:
                script_logger.error(
                    "Population failed during Phase 4: Dependency Population. Review logs."
                )
                # Decide if this should be a sys.exit(1) based on criticality
            else:
                script_logger.info("Phase 4: Dependency Population processing completed.")

        # --- Phase 5: Populate StatementGroup Dependencies ---
        script_logger.info("--- Executing Phase 5: Populate StatementGroup Dependencies ---")
        if not success_phase3 or not success_phase4: # Or specific checks if these phases must succeed
            script_logger.warning(
                "Skipping Phase 5: StatementGroup Dependency Population due to failures or incomplete data in prior phases."
            )
        else:
            success_phase5 = populate_statement_group_dependencies(
                SessionLocal,
                args.batch_size,
                args.create_tables, # Pass create_tables to allow skipping pre-fetch if tables new
            )
            if not success_phase5:
                script_logger.error("Population failed during Phase 5: StatementGroup Dependency Population. Review logs.")
                # Decide if this should be a sys.exit(1)
            else:
                script_logger.info("Phase 5: StatementGroup Dependency Population processing completed.")

        script_logger.info("--- Database Population Script Finished ---")

    except SQLAlchemyError as e:
        script_logger.critical("A top-level database error occurred: %s", e, exc_info=True)
        sys.exit(1)
    except FileNotFoundError as e: # Should be caught by earlier checks, but as a fallback
        script_logger.critical("A required file was not found: %s", e, exc_info=True)
        sys.exit(1)
    except Exception as e:  # pylint: disable=broad-except
        script_logger.critical(
            "An unexpected critical error occurred in main script execution: %s", e, exc_info=True
        )
        sys.exit(1)
