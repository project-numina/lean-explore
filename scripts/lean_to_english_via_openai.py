...
try:
    ...
    from dev_tools.config import APP_CONFIG, get_openai_api_key

    from dev_tools.llm_caller import OpenAIClient, LLMCostTracker
...
async def _process_single_statement_group_with_llm(
    ...
    Args:
        ...
        generated_group_translations: Cache of recently generated descriptions.
        client: The OpenAIClient instance.
    ...
    try:
        ...
        if not generation_model_name:
            logger.error(
                ...
                "Cannot initialize OpenAIClient."
            )
            return False  # Abort if model name isn't found in the loaded config

        cost_tracker = LLMCostTracker(model_costs_override=APP_CONFIG.get("costs"))
        client = OpenAIClient(
            api_key=api_key,
        ...
        # If OpenAIClient sets an attribute like 'self.default_generation_model',
        # this will show it.
        logger.info(
            "OpenAIClient initialized to use Generation Model: %s",
            generation_model_name,
        )

    except (
        ValueError
    ) as ve:  # Catch the specific ValueError from OpenAIClient if it still occurs
        logger.error(
            "Failed to initialize OpenAIClient: %s. This might happen if the "
            "model name is invalid or other internal OpenAIClient setup fails.",
            ve,
            exc_info=True,
        )
        logger.error(
        ...
        logger.error(
            "An unexpected error occurred while initializing OpenAIClient: %s",
            e,
            exc_info=True,
        )
        return False
