...
class TestEnsureConfigDirExists:
    ...
    def test_directory_creation_and_idempotency(
        self, isolated_config_dir: pathlib.Path
    ):
...
class TestLoadConfigData:
    ...
    def test_load_non_existent_file(self, tmp_path: pathlib.Path):
        """Ensures loading from non-existent file returns empty dict.
    ...
    def test_load_corrupted_toml_file(
        ...
        """Checks loading a corrupted TOML file returns empty dict and logs warning.
        ...
        corrupted_toml_file.write_text(
            "this is not valid toml content {=}", encoding="utf-8"
        )
    ...
    def test_load_exception_during_open(
        ...
        def mock_open_raises_exception(*args, **kwargs):
            ...
            raise OSError("Test-induced file read error")
...
class TestApiKeyManagement:
    ...
    def test_save_and_load_new_key(
        ...
        assert (
            toml.loads(config_file.read_text(encoding="utf-8"))[section_name][key_name]
            == api_key_value
        )
    ...
    def test_save_invalid_key(
        ...
        """Checks saving None/empty string as a key fails and logs an error.
        ...
        with caplog.at_level(logging.ERROR):
            ...
            assert (
                f"Attempted to save an invalid or empty {key_type_name} API key."
                in caplog.text
            )
            caplog.clear()
    ...
    def test_load_key_section_or_key_not_exists_in_file(
        ...
        """Verifies loading when section or key missing from config file.
        ...
        # Save an unrelated key to ensure the file exists but doesn't
        # contain the target key/section.
    ...
    def test_load_key_invalid_type_in_file(
        ...
        """Checks loading when a key exists but has an incorrect data type.
        ...
        config_file.write_text(
            toml.dumps({section_name: {key_name: 12345}}), encoding="utf-8"
        )
    ...
    def test_delete_existing_key_and_empty_section(
        ...
        """Verifies deleting existing key also removes its section if empty.
    ...
    def test_delete_key_file_not_exists_succeeds(
        ...
        """Checks deleting a key when config file doesn't exist is a no-op.
    ...
    def test_save_failure_after_key_deletion_in_memory(
        ...
        """Tests behavior when saving config file fails after in-memory key deletion.
        ...
        monkeypatch.setattr(
            "lean_explore.cli.config_utils._save_config_data", lambda path, data: False
        )
...
class TestOriginalGetConfigFilePath:
    """Tests for `get_config_file_path` without fixture-based monkeypatching.
    ...
    def test_path_structure_with_mocked_home(self, monkeypatch: pytest.MonkeyPatch):
        """Verifies config file path relative to a mocked home directory.
        ...
        monkeypatch.setattr(
            "os.path.expanduser",
            lambda path_str: mock_home if path_str == "~" else path_str,
        )
