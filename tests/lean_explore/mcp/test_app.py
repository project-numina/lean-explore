...
from mcp.server.fastmcp import Context as MCPContext # Import MCPContext is not needed here
from lean_explore.mcp.app import mcp_app, AppContext
...
class TestFastMCPAppLifespan:
    ...
    async def test_app_lifespan_initializes_backend_service_api(self, mocker: "MockerFixture"):
    ...
    async def test_app_lifespan_initializes_backend_service_local(self, mocker: "MockerFixture"):
    ...
    async def test_app_lifespan_raises_error_if_backend_not_set(self, mocker: "MockerFixture"):
        """Tests that the lifespan context raises RuntimeError if no backend service is set.
        ...
        # Mock getattr to simulate the backend service not being found on the app instance
        ...
        assert "Ensure the server script correctly sets the backend service attribute" in str(exc_info.value)

    async def test_app_lifespan_with_non_none_backend_type(self, mocker: "MockerFixture"):
        """Verifies lifespan correctly handles a backend that is not None but not a recognized type.

        While the main app logic handles `None`, this test ensures robustness
        if a different, unexpected object is set as the backend.
        """
        mock_unrecognized_backend = MagicMock() # A generic mock without a specific spec
    ...
    async def test_app_context_dataclass(self):
        ...
        # Test that the attribute can be reassigned (dataclasses are not frozen by default)
