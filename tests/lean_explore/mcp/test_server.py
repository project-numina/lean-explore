...
class TestServerArguments:
    ...
    def test_parse_arguments_backend_required(self, backend_choice: str):
        ...
        test_args = (
            ["--api-key", "dummy", "--log-level", "WARNING"]
            if backend_choice == "api"
            else ["--log-level", "WARNING"]
        )
        with patch.object(sys, "argv", ["server_script.py"] + test_args), pytest.raises(
            SystemExit
        ) as sysexit:  # argparse raises SystemExit on error
            parse_arguments()
    ...
    def test_parse_arguments_api_key_required_for_api_backend(self):
        ...
        with patch.object(sys, "argv", ["server_script.py"] + test_args), patch(
            "builtins.print"
        ) as mock_print, pytest.raises(SystemExit) as sysexit:
...
class TestServerMainFunction:
    ...
    def setup_main_mocks(self, mocker):
        ...
        self.mock_local_service_init = mocker.patch(
            "lean_explore.local.service.Service"
        )

        # Mock defaults paths and existence checks for local backend pre-checks
        self.mock_default_db_path = mocker.patch.object(
            lean_explore.defaults, "DEFAULT_DB_PATH", new_callable=MagicMock
        )
        self.mock_default_faiss_index_path = mocker.patch.object(
            lean_explore.defaults, "DEFAULT_FAISS_INDEX_PATH", new_callable=MagicMock
        )
        self.mock_default_faiss_map_path = mocker.patch.object(
            lean_explore.defaults, "DEFAULT_FAISS_MAP_PATH", new_callable=MagicMock
        )
        self.mock_lean_explore_toolchains_base_dir = mocker.patch.object(
            lean_explore.defaults,
            "LEAN_EXPLORE_TOOLCHAINS_BASE_DIR",
            new_callable=MagicMock,
        )
        self.mock_default_active_toolchain_version = mocker.patch.object(
            lean_explore.defaults, "DEFAULT_ACTIVE_TOOLCHAIN_VERSION", "test_version"
        )
        ...
        (
            self.mock_lean_explore_toolchains_base_dir.return_value.__truediv__.return_value.resolve.return_value
        ) = Path("/tmp/test_toolchain")

    def test_main_runs_api_backend_successfully(self):
        """Verifies successful main function init and run with API backend."""
    ...
    def test_main_exits_if_api_key_missing_for_api_backend(self):
        """Verifies main exits if API key is missing for API backend."""
        self.mock_sys_argv.extend(["--backend", "api"])  # Missing --api-key

        main()

        self.mock_error_console_print.assert_not_called()
        # Use patch('builtins.print') for exact stderr message verification
    ...
    def test_main_exits_if_local_backend_files_missing(self):
        """Verifies main exits if essential local backend files are missing."""
        self.mock_sys_argv.extend(["--backend", "local", "--log-level", "INFO"])
        self.mock_default_db_path.exists.return_value = (
            False  # Simulate missing DB file
        )
        ...
        assert (
            ...
            in self.mock_error_console_print.call_args[0][0]
        )
        assert (
            "Please run `leanexplore data fetch`"
            in self.mock_error_console_print.call_args[0][0]
        )
    ...
    def test_main_handles_mcp_app_run_exception(self):
        ...
        self.mock_sys_exit.assert_called_once_with(1)
        assert any(
            "MCP server exited with an unexpected error: MCP app crashed"
            in record.message
            for record in self.mock_logging_config.call_args[0]
            if isinstance(record, logging.LogRecord)
        )
