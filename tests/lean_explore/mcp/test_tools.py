...
class TestSearchTool:
    ...
    async def test_search_successful_no_filters_no_limit(
        ...
        mock_backend_response = APISearchResponse(
            ...
            results=[
                APISearchResultItem(
                    id=1,
                    primary_declaration=APIPrimaryDeclarationInfo(lean_name="Foo"),
                    statement_text="def Foo := ...",
                    display_statement_text="def Foo := ... disp",
                    source_file="Foo.lean",
                    range_start_line=1,
                ),
                APISearchResultItem(
                    id=2,
                    primary_declaration=APIPrimaryDeclarationInfo(lean_name="Bar"),
                    statement_text="def Bar := ...",
                    display_statement_text="def Bar := ... disp",
                    source_file="Bar.lean",
                    range_start_line=1,
                ),
            ],
            count=2,
        ...
        if asyncio.iscoroutinefunction(backend_service_mock.search):
            backend_service_mock.search.assert_awaited_once_with(
    ...
    async def test_search_successful_with_package_filters_and_limit(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
        ...
        mock_backend_response = APISearchResponse(
            ...
            results=[
                APISearchResultItem(
                    ...
                    statement_text="def Nat.prime := ...",
                    display_statement_text="def Nat.prime := ... disp",
                    source_file="Mathlib/Data/Nat/Prime.lean",
                    range_start_line=100,
                ),
                APISearchResultItem(
                    id=11,
                    primary_declaration=APIPrimaryDeclarationInfo(
                        lean_name="prime_factorization"
                    ),
                    statement_text="def prime_factorization := ...",
                    display_statement_text="def prime_factorization := ... disp",
                    source_file="Mathlib/NumberTheory/Prime.lean",
                    range_start_line=200,
                ),
            ],
            count=2,
            total_candidates_considered=2,
        ...
        result = await search(
            ctx, query=query, package_filters=package_filters, limit=limit
        )
    ...
    async def test_search_returns_empty_if_backend_returns_none(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
    ...
    async def test_search_handles_empty_results_from_backend(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
...
@pytest.mark.asyncio
async def test_search_raises_error_if_backend_has_no_search_method_outside_class(
    mock_mcp_context: MCPContext,
):
    ...
    mock_mcp_context.request_context.lifespan_context.backend_service = (
        mock_ill_formed_backend
    )

    with pytest.raises(
        RuntimeError, match="Search functionality not available on configured backend."
    ):
...
class TestGetByIdTool:
    ...
    async def test_get_by_id_successful(
        ...
        mock_backend_item = APISearchResultItem(
            id=group_id,
            primary_declaration=APIPrimaryDeclarationInfo(lean_name="Lean.Definition"),
            statement_text="def Lean.Definition := ...",
            display_statement_text="def Lean.Definition := ... disp",
            source_file="Lean.lean",
            range_start_line=50,
        )
    ...
    async def test_get_by_id_propagates_backend_exceptions(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
        """Verifies `get_by_id` propagates exceptions raised by the backend service."""
        ctx = set_backend_on_context
        group_id = 500
        backend_service_mock.get_by_id.side_effect = RuntimeError(
            "Backend get_by_id failed"
        )
...
class TestGetDependenciesTool:
    ...
    async def test_get_dependencies_successful_with_citations(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
        ...
        mock_backend_response = APICitationsResponse(
            ...
            citations=[
                APISearchResultItem(
                    id=701,
                    primary_declaration=APIPrimaryDeclarationInfo(lean_name="Dep.One"),
                    statement_text="def Dep.One := ...",
                    display_statement_text="def Dep.One := ... disp",
                    source_file="Dep1.lean",
                    range_start_line=10,
                ),
                APISearchResultItem(
                    id=702,
                    primary_declaration=APIPrimaryDeclarationInfo(lean_name="Dep.Two"),
                    statement_text="def Dep.Two := ...",
                    display_statement_text="def Dep.Two := ... disp",
                    source_file="Dep2.lean",
                    range_start_line=20,
                ),
            ],
            count=2,
        ...
        if asyncio.iscoroutinefunction(backend_service_mock.get_dependencies):
            backend_service_mock.get_dependencies.assert_awaited_once_with(
                group_id=group_id
            )
        else:
    ...
    async def test_get_dependencies_successful_no_citations(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
    ...
    async def test_get_dependencies_not_found(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
        """Verifies `get_dependencies` returns None if the backend returns None.
    ...
    async def test_get_dependencies_propagates_backend_exceptions(
        self, set_backend_on_context: MCPContext, backend_service_mock: MagicMock
    ):
        """Verifies `get_dependencies` propagates exceptions."""
        ctx = set_backend_on_context
        group_id = 501
        backend_service_mock.get_dependencies.side_effect = Exception(
