...
import hashlib
import logging
from pathlib import Path # Not directly used, but good for type hinting if needed
            ...
            while scan_index < n:
                ...
                if char == "[":
                    ...
                    if bracket_level == 0:
                        ...
                        while scan_index < n and text[scan_index].isspace(): # Skip trailing whitespace
                            scan_index += 1
                        i = scan_index
                        break
                scan_index += 1
...
def _calculate_text_hash(text: str) -> str:
    """Calculates SHA256 hash for a given text string.
...
def _choose_primary_declaration(
    ...
    candidates = [
        decl for decl in declarations_in_block if not decl.is_internal
    ]
    ...
    if candidates:
        ...
        logger.debug(
            "  Chosen primary: '%s' (Type: %s, Internal: %s) from %d declarations in block.",
    ...
    logger.warning(
        "  Could not choose a primary declaration from %d declarations using current heuristic.",
        len(declarations_in_block),
    )
    return None
...
def phase3_group_statements(
    ...
    try:
        with session_factory() as session:
            ...
            # Select declarations that haven't been grouped and have necessary source info
            stmt = (
                select(Declaration)
    ...
    for decl in declarations_to_process:
        ...
        if not all([decl.source_file, decl.range_start_line is not None,
                    decl.range_start_col is not None, decl.range_end_line is not None,
                    decl.range_end_col is not None, decl.statement_text is not None]):
            logger.warning(
                "Declaration ID %d ('%s') missing required block info. Skipping.",
                decl.id, decl.lean_name
            )
            continue
        block_key = (
            decl.source_file,
            decl.range_start_line, decl.range_start_col,
            decl.range_end_line, decl.range_end_col,
    ...
    with tqdm(total=len(blocks_map), desc="Phase 3: Grouping", unit="block") as pbar:
        for block_key_tuple, decls_in_block in blocks_map.items():
            pbar.update(1)
            (block_src_file, blk_start_line, blk_start_col,
             blk_end_line, blk_end_col, blk_stmt_text) = block_key_tuple
            ...
            try:
                ...
                if not primary_decl or primary_decl.id is None:
                    logger.warning(
                        "  Could not determine valid primary declaration for block hash %s. Skipping.",
                        block_hash[:8]
                    )
                    continue

                # Determine display text: use signature if available, else full block text
                # Then clean attributes and comments for display.
                base_display_text = primary_decl.declaration_signature if \
                                    primary_decl.declaration_signature and \
                                    primary_decl.declaration_signature.strip() else blk_stmt_text
                cleaned_display_text = _remove_attributes_from_text(base_display_text)
                cleaned_display_text = _remove_all_comments_from_text(cleaned_display_text)
                ...
                with session_factory() as session:
                    ...
                    if sg_entry: # Update existing StatementGroup
                        ...
                        if sg_entry.primary_decl_id != primary_decl.id:
                            ...
                            needs_update = True
                        if sg_entry.docstring != primary_decl.docstring: # Handles None comparison
                        ...
                        new_sg = StatementGroup(
                            text_hash=block_hash, statement_text=blk_stmt_text,
                            display_statement_text=cleaned_display_text,
                            source_file=block_src_file,
                            range_start_line=blk_start_line, range_start_col=blk_start_col,
                            range_end_line=blk_end_line, range_end_col=blk_end_col,
                        ...
                        except IntegrityError: # Handle rare race if hash collision by other means
                            session.rollback()
                            logger.warning("IntegrityError on new SG commit (hash %s). Refetching.", block_hash[:8])
                            sg_refetched = session.execute(
                                select(StatementGroup).filter_by(text_hash=block_hash)
                            ).scalar_one_or_none()
                            if sg_refetched: sg_id_for_linking = sg_refetched.id
                            else: logger.error("Refetch failed for hash %s. Skipping linking.", block_hash[:8])

                    # Link declarations to this StatementGroup
                    if sg_id_for_linking is not None:
                        for decl_to_link in decls_in_block:
                            if decl_to_link.id is not None and \
                               decl_to_link.statement_group_id != sg_id_for_linking:
                                declaration_update_batch.append({
                                    "id": decl_to_link.id,
                                    "statement_group_id": sg_id_for_linking,
                                })
            except SQLAlchemyError as e_block_db:
                logger.error("DB error processing block (hash %s): %s. Rolling back session for this block.",
                             block_hash[:8] if 'block_hash' in locals() else 'N/A', e_block_db)
                if 'session' in locals() and session.is_active: session.rollback()
            except Exception as e_block_unexpected: # pylint: disable=broad-except
                logger.error("Unexpected error processing block (hash %s): %s",
                             block_hash[:8] if 'block_hash' in locals() else 'N/A', e_block_unexpected, exc_info=True)
    ...
    if declaration_update_batch:
        logger.info(
            ...
            len(declaration_update_batch), batch_size
        )
        try:
            with session_factory() as session:
                ...
                for i in range(0, len(declaration_update_batch), batch_size):
                    ...
                    if batch_payload:
                        ...
                        if ((i // batch_size) + 1) % 10 == 0 or (i + batch_size) >= len(declaration_update_batch):
                            logger.info("  Committed link batch %d, total links committed: %d",
                                        (i // batch_size) + 1, committed_links_total)
                links_to_commit_count = committed_links_total # Reflect actual committed
    ...
    logger.info(
        ...
        len(blocks_map), created_group_count, updated_group_count, links_to_commit_count
    )
    return True